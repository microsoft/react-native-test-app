...
async function copyProjectTemplateAndReplace(
    srcRootPath,
    destPath,
    ...
    fs.readFile(manifestFilePath, function (error, content) {
        ...
        if (resourcesPaths && !Array.isArray(resourcesPaths)) {
            resourcesPaths = resourcesPaths.windows ? resourcesPaths.windows : [];
        } else if (!resourcesPaths) {
            resourcesPaths = [];
        }
    ...
    for (const mapping of projectFilesMappings) {
        copyAndReplace(mapping.from, destPath, mapping.to, projectFilesReplacements);
    }
