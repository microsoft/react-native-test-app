...
export function resolveContents(
    ...
    return Object.keys(replacements).reduce((_, regex) => {
        return _.replace(new RegExp(regex, 'g'), replacements[regex]);
    }, content);
}
...
function copyAndReplace(
    ...
    if (binaryExtensions.indexOf(extension) !== -1) {
        ...
        fs.writeFile(fullDestPath, content, {
            encoding: 'utf8',
            mode: srcPermissions,
        }, err => {
            if (err) {
...
export function copyAndReplaceAll(
    ...
    return Promise.all(walk(srcPath).map((absoluteSrcFilePath) => {
...
export function linkSourceFiles(
    srcPath,
    destPath,
    relativeDestDir
) {
...
export async function copyProjectTemplateAndReplace(srcRootPath, destPath, reactNativeModulePath, testAppModulePath) {
    if (!srcRootPath) {
    ...
    fs.mkdirSync(path.join(destPath, windowsDir, projDir, 'Bundle'), { 'recursive': true });

    const manifestFilePath = findClosestPathTo('app.json');

    //Read path to resources from manifest and copy them
    fs.readFile(manifestFilePath, function (error, content) {
        ...
        for (const resource of resourcesPaths) {
            copyAndReplaceAll(path.relative(path.dirname(manifestFilePath), resource), destPath, path.join(windowsDir, projDir, 'Bundle', path.basename(resource)));
        }
    });

    const replacements = {
        '\\$\\(ManifestRootPath\\)': path.relative(path.join(destPath, windowsDir, projDir), path.dirname(manifestFilePath)),
        '\\$\\(ReactNativeModulePath\\)': path.relative(path.join(destPath, windowsDir, projDir), reactNativeModulePath),
        '\\$\\(SourceFilesPath\\)': path.relative(path.join(destPath, windowsDir, projDir), path.join(srcRootPath, projDir)),
    };
...
copyProjectTemplateAndReplace(srcRootPath, destPath, reactNativeModulePath, testAppNodeModulePath);
