...
function setup(platform: Platform, workingDir: string) {
  mkdir_p(workingDir);
  ...
  // Bundle before asset discovery, and ship local JS rather than relying on
  // Metro in a release app.
...
function validateAndroid(workingDir: string) {
  ...
  run("bundletool", [
    ...
    `--ks-pass=file:${path.join(workingDir, "signing-password")}`,
    `--key-pass=file:${path.join(workingDir, "signing-password")}`,
    "--overwrite",
  ]);
...
function validateIOS(workingDir: string) {
  ...
  const packagedFiles = glob(app);
  const plists = glob(app, "**/{*,.*}.{plist,xcprivacy}");
  for (const plist of plists) {
    run("plutil", ["-lint", plist]);
  }

  // Inspect actual Mach-O load commands: arm64 alone also matches simulators.
  const binaryCount = packagedFiles.reduce((count, binary) => {
